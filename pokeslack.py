...
class Pokeslack:
    ...
    def try_send_pokemon(self, pokemon, debug):
        ...
        logger.info("Falloff is set to " + str(Pokeconfig.falloff))
        if pokemon.expires_in().total_seconds() < travel_time and Pokeconfig.falloff == True:
        ...
        message = '<' + Pokeconfig.slack_name + '> I found a <%s|%s> %s <%s|%s away> expiring in %s%s' % (pokedex_url, pokemon.name, stars, map_url, miles_away, time_remaining, from_lure)
